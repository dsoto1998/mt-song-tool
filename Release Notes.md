...
- **Fit to screen on double-click** — Double-clicking the drag zone at the top of the window resizes it to the full available screen height (respecting the menu bar and Dock), without entering full-screen mode.

### v1.2.2 — April 7, 2026

#### UI

- **San Francisco font** — All UI text now uses the system San Francisco font (previously Lato). The MTST logo retains Horizon.
- **Aligned copy buttons** — Copy-to-clipboard buttons in the Locators and Time Signatures panels are now consistently column-aligned across all rows, regardless of timecode width.
- **No tab bar** — The View > Show Tab Bar menu item is suppressed; the window no longer supports macOS native tab groups.
- **Help > Release Notes** — Release notes are now accessible from the Help menu and open in a dedicated window.
