...
- `sessionToken: UUID` — `@Published private(set)`; bumped by `fullReset()`. `EditView` observes via `.onChange(of: editPlayer.sessionToken)` to reset its `@State` (selections, zoom, padding, rowHeights).
- `fullReset()` — called by `clearAll()` and `loadNewFile()` in ContentView. Calls `stop()` + `loadStems([])`, then zeroes `editableTempoEvents`, `locatorOverrides`, `timeSigOverrides`, `busAlignmentResult`, `isSessionDirty`, and bumps `sessionToken`. Do NOT call from `reparseAfterSave()` — that must preserve stems.
...
- **All FFmpeg export paths use `-ac 2`** — `convertNonConforming`, `applyEdits` (multi-segment, cut-segments, legacy no-cuts), and `exportStems` (multi-segment, simple) all force 2-channel stereo output. Mono stems are upmixed to dual-mono. Do not remove `-ac 2` from these call sites.
...
- **`clearAll()` and `loadNewFile()` call `editPlayer.fullReset()`** — fully wipes Edit tab state (tempo events, locator overrides, alignment result, stems, session dirty flag, `sessionToken` bump). `reparseAfterSave()` does NOT call `fullReset()` — stems must survive a save re-parse.
- **`parsedResult?.file` onChange seeds tempo events** — `editPlayer.seedTempoEvents(result.tempoEvents)` is called in the `parsedResult?.file` onChange block (EditView) so tempo map is correct when .als parses after stems load. Also called in `.onChange(of: stemURLs)` for the stems-first case.
